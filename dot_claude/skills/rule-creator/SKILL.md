---
name: rule-creator
description: Claude Codeのルールファイル（.claude/rules/*.md）を追加・編集するスキル。プロジェクトレベル（.claude/rules/）またはユーザーレベル（~/.claude/rules/、chezmoi管理）への配置を選択可能。ルールの新規作成、既存ルールの変更、パス条件付きルールの設定など、.claude/rulesの管理全般でトリガーする。
---

# Add Rule

Claude Codeの `.claude/rules/` ルールファイルを追加・編集するスキル。プロジェクトレベルとユーザーレベルのどちらにインストールするか選択できる。
...
| スコープ | 配置先 | 効果範囲 | 管理方法 |
|---------|--------|---------|---------|
| **プロジェクト** | `<プロジェクトルート>/.claude/rules/` | そのプロジェクトでのみ有効 | 直接配置（gitで管理） |
| **ユーザー** | `~/.claude/rules/` | 全プロジェクトで有効 | chezmoi経由（`dot_claude/rules/`） |

- ユーザーが明示的に指定した場合はそれに従う
- 指定がない場合はAskUserQuestionツールで確認する。判断の目安:
  - そのプロジェクト固有の規約 → **プロジェクト**
  - 個人の好みや全プロジェクト共通のルール → **ユーザー**
...
- ルールの内容
- パス条件の要否
- **インストールスコープ**（プロジェクト or ユーザー）

既存ルールを確認し、同一トピックがあれば新規作成ではなく編集を提案する:

```bash
# プロジェクトレベル
ls .claude/rules/ 2>/dev/null || echo "プロジェクトルールはまだ存在しません"

# ユーザーレベル（chezmoi管理）
ls dot_claude/rules/ 2>/dev/null || echo "ユーザールールはまだ存在しません"
```

### 2. ルールファイルの作成・編集

スコープに応じて適切なディレクトリにファイルを作成または編集する:

- **プロジェクト**: `<プロジェクトルート>/.claude/rules/<name>.md`
- **ユーザー**: chezmoiソースディレクトリの `dot_claude/rules/<name>.md`

### 3. diffプレビューと承認

変更内容をユーザーに提示する。以下を伝えること:
- インストールスコープ（プロジェクト / ユーザー）
- デプロイ先パス
- ルール内容の要約
...
- **プロジェクト**: 直接配置済みのため追加手順不要
- **ユーザー**: `chezmoi apply` を実行してデプロイする
