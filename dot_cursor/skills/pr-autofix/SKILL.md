...
```bash
bun ~/.cursor/skills/pr-autofix/scripts/collect.ts [<PR番号|URL>]
```
...
- 「エラー行を抽出できず」と出た失敗ジョブがあれば、そのジョブだけ `general-purpose` に `ci-failures.json` の該当 `log_excerpt` を読ませ、エラー本文と `file:line` を要約させる
