...
/**
 * PR の CI 失敗とレビューコメントを1プロセスで並列収集する。
 *
 * JSON 全文はワークスペースのファイルに書き、標準出力には件数と要約だけを出す。
 * 呼び出し元の司令塔モデルが標準出力だけで修正計画を立てられる分量に抑え、
 * 全文が必要になったときだけサブエージェントにファイルを読ませるための分担。
 */
...
const USAGE =
  "Usage: bun collect.ts [<pr-number|pr-url>] [--repo <owner/repo>] [--out <dir>]";

function parseArgs(argv: string[]): Args {
  const args: Args = {};
  for (let i = 0; i < argv.length; i++) {
    const arg = argv[i];
    if (arg === "--repo") args.repoFlag = argv[++i];
    else if (arg === "--out") args.out = argv[++i];
    else if (arg === "--help" || arg === "-h") {
      console.error(USAGE);
      process.exit(0);
    } else if (!args.ref) args.ref = arg;
  }
  return args;
}
...
async function collectCi(repo: string, prNumber: number) {
  const checksJson = await $`gh pr checks ${String(prNumber)} --repo ${repo} --json name,state,bucket,workflow,link,description,completedAt`
    .nothrow()
    .text();
  const checks = JSON.parse(checksJson.trim() || "[]") as Check[];
...
async function main() {
  ...
  if (!urlMatch) {
    console.error(`PR の URL から owner/repo を判定できません: ${pr.url}`);
    process.exit(1);
  }
  ...
  for (const check of ci.failed_checks) {
    ...
    if (check.log_fetch_error) {
      ...
      out.push(
        "    エラー行を抽出できず。ci-failures.json の log_excerpt をサブエージェントに読ませる",
      );
    } else {
