...
if (body.startsWith("🤖 Codex says:")) {
  process.exit(0);
}

// body がシェル変数・コマンド置換の場合、展開前の文字列なので
// コマンド全体にプレフィックスが含まれているかで判定する
if (/^\$/.test(body) && command.includes("🤖 Codex says:")) {
  process.exit(0);
}
...
console.log(
  JSON.stringify({
    decision: "block",
    reason:
      "GitHub PR コメントの本文は「🤖 Codex says:」で始める必要があります（github-review.md ルール）。body の先頭にプレフィックスを追加してください。",
  }),
);
