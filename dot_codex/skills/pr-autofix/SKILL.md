...
```bash
bun ~/.codex/skills/pr-autofix/scripts/collect.ts [<PR番号|URL>]
```
...
返信はエージェントの発言を引用記法で囲みます（`AGENTS.md`）。囲まないと `gh-comment-format-guard.ts` hook にブロックされます。署名行以降はすべて行頭を `>` で始めます。コミットハッシュは半角括弧で囲みます。
